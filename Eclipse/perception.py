...
# ---- 레이더 파라미터/제한들 ----
MIN_VALID_DEPTH = 0.5           # m
MAX_VALID_DEPTH = 120.0         # m (※ 최신본과 일관: 레이더 range 속성과 같은 값 사용)
BASE_MAX_AZIMUTH_DEG = 3.0      # deg (기본 허용 방위각)
MAX_AZIMUTH_DEG_LIMIT = 6.0     # deg (커브 시 상한)
MAX_DEPTH_JUMP_PER_SEC = 40.0   # 점프 상한
MIN_DOPPLER_ABS = 0.8           # m/s (정지물 억제용)
LANE_LATERAL_LIMIT_M = 2.0      # m: 차로-코리더(|lateral|<=2.0m) 게이팅
EMA_ALPHA_DIST = 0.30
EMA_ALPHA_RELSPEED = 0.30
RELSPEED_EPS = 0.30
TARGET_LOST_TIMEOUT_SEC = 0.60
MIN_CONFIRM_FRAMES = 2          # 연속 N프레임 업데이트 성공 시에 has_target=True

# ---- 시각화 관련(원본 스타일 유지) ----
COLOR_LANE = (0, 0, 255)       # RED
        ...
        while True:
            ...
            if _go['seen']:
                if sim_time < 10.0:
                    ...
                    lead_phase = 'PHASE2(STOP)'
                else:
                    lead_phase = 'PHASE3'

                if lead and (lead_phase != last_phase):
                    if lead_phase in ('PHASE1', 'PHASE3'):
                        if tm is not None:
                ...
                if lead and tm and lead_phase in ('PHASE1', 'PHASE3'):
